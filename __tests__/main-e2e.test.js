...
test('spawn the cli to process data and ensure markdown file is generated correctly', async () => {
  ...
  const projectFixtures = path.join(
    __dirname,
    './__fixtures__/main-datafiles/myapp/pages'
  )
  ...
  spawnedCliEnvironmentVariables.DEBUG = 'gigsboat:*'

  const cliData = spawnSync(
    cliExecutable,
    ['--source-directory', projectFixtures, '--output-file', projectOutputFile],
    { shell: true, env: spawnedCliEnvironmentVariables }
  )
